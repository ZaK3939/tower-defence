...
export class Sprite extends Phaser.Physics.Arcade.Sprite implements ISprite {
  ...
  private collisionHandler: Nullable<(tile: Phaser.GameObjects.Image) => void> =
    null;
  ...
  constructor(
    scene: IWorld,
    { texture, positionAtMatrix, health, speed, frame = 0 }: SpriteData
  ) {
  ...
  public getAllPositionsAtMatrix() {
    return this.getProjectionOnGround().map((point) =>
      Level.ToMatrixPosition(point)
    );
  }
  ...
  private getCollidedTile(direction: number) {
    ...
    const speedPerFrame =
      (this.speed / friction) *
      (WORLD_COLLIDE_SPEED_FACTOR * this.scene.deltaTime);
    const offset = this.scene.physics.velocityFromAngle(
      direction,
      speedPerFrame
    );
  ...
  private updateIndicators() {
    this.indicators.forEach((indicator, index) => {
      ...
      if (value <= 0.0) {
        ...
        bar.setSize(
          (indicator.container.width - 2) * value,
          indicator.container.height - 2
        );
      }
    });
  }
