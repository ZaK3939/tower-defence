...
export function registerWawaTexture(scene: Scene, wawa: Wawa) {
  {
    ...
    const shadows = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "shadows", wawa.tiers.legs))
    ctx.drawImage(replaceSwatch(shadows, "#000000,#000000"), 0, 0);
    const legs = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "legs", wawa.tiers.legs))
    ctx.drawImage(replaceSwatch(legs, wawa.swatches.legs), 0, 0);
    const body = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "body", 1))
    ctx.drawImage(replaceSwatch(body, wawa.swatches.body), 0, 0);
    const chest = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "chest", wawa.tiers.chest))
    ctx.drawImage(replaceSwatch(chest, wawa.swatches.chest), 0, 0);
    const headwear = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "headwear", wawa.tiers.headwear))
    ctx.drawImage(replaceSwatch(headwear, wawa.swatches.headwear), 0, 0);
    const eyes = scene.textures.get(factionNames[wawa.factionId]).get(frameKey(wawa.factionId, "eyes", wawa.tiers.eyes))
  ...
  if (wawa.petId > 0) {
    ...
    const shadows = scene.textures.get("pet").get("pet-" + wawa.petId + "-shadows" + ".png")
    ctx.drawImage(replaceSwatch(shadows, "#000000,#000000"), 0, 0);
    const body = scene.textures.get("pet").get("pet-" + wawa.petId + "-body" + ".png")
