...
export class Level extends TileMatrix implements ILevel {
  ...
  public hasTilesBetweenPositions(positionA: Vector2D, positionB: Vector2D) {
    ...
    return line.some(
      (point) => this.getTile({ ...point, z: 1 })?.tileType === TileType.MAP
    );
  }
  ...
  private addFalloffLayer(
    ...
    for (let y = 0; y < sizeInTiles; y++) {
      for (let x = 0; x < sizeInTiles; x++) {
        if (
          x < offset ||
          x >= sizeInTiles - offset ||
          y < offset ||
          y >= sizeInTiles - offset
        ) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.isFreePoint(tilePosition)) {
        ...
        const tile = this.scene.add.image(
          ...
          Phaser.Math.Between(
            0,
            LEVEL_PLANETS[this.planet].SCENERY_VARIANTS - 1
          )
        ) as ITile;
  ...
  static ToWorldPosition(tilePosition: Vector3D) {
    ...
    const positionAtWorld: Vector2D = {
      ...
      y:
        (tilePosition.x + tilePosition.y) * (height * origin) -
        tilePosition.z * (height * 0.5),
    };
  ...
  static GetTileDepth(YAtWorld: number, tileZ: number) {
    return (
      YAtWorld + tileZ * LEVEL_TILE_SIZE.height + LEVEL_TILE_SIZE.height * 0.5
    );
  }
}
